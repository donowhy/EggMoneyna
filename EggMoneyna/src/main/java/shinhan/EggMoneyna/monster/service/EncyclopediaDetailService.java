...
public class EncyclopediaDetailService {
    ...
    public List<EncyclopediaDetail> save(Long id) {
        ...
        String[] MONG_NAMES = {
                ...
                "미스터리 아티스트 `리노 (RINO)`",
                "수줍은 디지털 크리에이터 `슈(SHOO)`",
                "We Will Rock you! 3인조 락 밴드 `도레미(DO RE MI)'",
                "바닷가의 쌍둥이 서퍼, `루루라라(LULU LALA)`",
                "에너제틱 행동파 라쿤 `플리 (PLI)`",
                "세상 쿨한 힙합 부엉이 `레이(LAY)`"
        };
        ...
        MonsterEncyclopedia monsterEncyclopedia = monsterEncyclopediaRepository.findById(id).orElseThrow();
        ...
        for (int i = 0; i < MONG_NAMES.length; i++) {
            ...
            detail.setMonsterEncyclopedia(monsterEncyclopedia);
